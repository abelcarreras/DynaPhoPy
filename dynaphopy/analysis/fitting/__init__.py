...
def phonon_fitting_cross_analysis(original, ps_frequencies, cross_list,
                            harmonic_frequencies=None,
                            thermal_expansion_shift=None,
                            fitting_function_type=0,
                            show_plots=True,
    ...
    for i in range(original.shape[1]): # loop para cada fonón
        ...
        #Print section
        n = len(cross_list)
        first_index = cross_list[math.floor(i/n)]
        second_index = cross_list[i-math.floor(i/n)*n]
        print ('\nCross Correlation Phonons # {0} and {1}'.format(first_index + 1, second_index + 1))
        print ('----------------------------------------------')
        print ('Width                      {0:15.6f} THz'.format(width))
        print ('Position                   {0:15.6f} THz'.format(position))
        ...
        if show_plots:
            ...
            plt.suptitle('Phonons # {0} and {1}'.format(first_index + 1, second_index + 1))
            plt.text(position+width, guess_height/2, 'Width: ' + "{:10.4f}".format(width),
                     fontsize=12)
...
def normalization(dt_Q2_s):
    normalized_QS = dt_Q2_s / np.sum(dt_Q2_s)
    print(normalized_QS)